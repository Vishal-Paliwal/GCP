...
my_options = {'--project=automatic-asset-253215',
              '--job_name=xfm-vustgclic-custsorgext-to-imcustomerattributeref---tryy',
...
def lookup(row, cust_ids):
    ...
    try:
        ...
        delete_query = """
        DELETE FROM `automatic-asset-253215.CORE.IM_CUSTOMER_ATTRIBUTE_REF`
        WHERE CUSTOMER_ID = ?"""
        query_params = [
            bigquery.ScalarQueryParameter(None, "INT64", result['CUSTOMER_ID']),
            ]
        job_config = bigquery.QueryJobConfig()
        job_config.query_parameters = query_params
        client.query(delete_query, job_config=job_config,)
        print('AFTER DELTEING UPDATING RECORDS AREE..........!!!!!')
        return result
    except KeyError as InsertRecords:
        print('INSIDE INSERT FUN...........!!!!!!!!!')
        return result
...
def run():
    ...
    (p | 'Read from custorgext' >> beam.io.Read(beam.io.BigQuerySource(
       ...
       | 'Insert/Update' >> beam.io.WriteToBigQuery(
            output_table,
