...
dataflow_options = {'--project=automatic-asset-253215',
                    '--job_name=xfm-vustgclic-custscontact-to-imcustomerattributeref',
...
FROM
  `automatic-asset-253215.STAGE.STG_CLIC_CUSTSCONTACT` b"""
    ...
    src_pipeline = 'IM_Table'
    IM_Table = p | 'Read From IM Table' >> beam.io.Read(beam.io.BigQuerySource(
        query=IM_data_query,
        use_standard_sql=True))

    join_pipeline = 'contact_table'
    contact_table = (p | 'Read From Contact Table' >> beam.io.Read(beam.io.BigQuerySource(
        query=contact_data_query, use_standard_sql=True))
                     )

    common_key = 'CUSTOMER_ID'
    pipelines_dictionary = {src_pipeline:IM_Table,join_pipeline:contact_table}

    join_data = (pipelines_dictionary
                 | 'Left join' >> LeftJoin(src_pipeline,IM_Table,join_pipeline,contact_table,common_key)
