...
#     Description     :     This Script updates records into the IM_CUSTOMER_ADDRESS_SCD_GCP Table  
...
def lookup2(row, cust_ids):
    result = row.copy()
    try:
        result.update(cust_ids[str(row['SOURCE_CREATE_DT'])+str(row['ADDR_LINE1_TXT'])+str(row['ADDR_LINE2_TXT'])+
        str(row['CITY_NAME'])+str(row['STATE_CODE'])+str(row['COUNTRY_CODE'])+str(row['POSTAL_ZIP'])+
        str(row['POSTAL_ZIP4'])+str(row['ADDR_CLEANSING_IND'])+str(row['ADDR_FRAUD_IND'])+str(row['ADDR_QAS_VERIFIED_IND'])+
        str(row['ADDR_TYPE_CODE'])+str(row['SHIPTO_FIRST_NAME'])+str(row['SHIPTO_LAST_NAME'])])
    except KeyError as InsertRecords:
        return result


def run():
    address_scd = """SELECT * FROM `hsni-dev-edw.EDW_STG_US.IM_CUSTOMER_ADDRESS_SCD_GCP`"""

    upd_addrorg_data = """SELECT
  CAST(a.HSN_ACCT_NUM AS INT64) AS CUSTOMER_ID,
  a.ADDRESS_NAME AS ADDR_NAME,
  'CLIC' AS ETL_SOURCE_SYSTEM,
   CAST(FORMAT_DATETIME('%Y-%m-%dT%H:%M:%S', CURRENT_DATETIME()) AS TIMESTAMP )AS ETL_END_EFFECTIVE_DT,
  '0' AS ETL_CURRENT_IND,
  CAST(FORMAT_DATETIME('%Y%m%d%H%M%S', CURRENT_DATETIME()) AS INT64) AS UPD_BATCH_NBR
FROM
  `hsni-dev-edw.EDW_STG_US.STG_CLIC_CUSTADDRORG` a"""
  ...
  a.ADDRESS_NAME AS ADDR_NAME,
  'CLIC' AS ETL_SOURCE_SYSTEM,
  PARSE_DATETIME('%Y%m%d%H%M%S', a.ROW_CREATED_DATE) AS SOURCE_CREATE_DT,
  ...
  TIMESTAMP_ADD ( CAST(FORMAT_DATETIME('%Y-%m-%dT%H:%M:%S', CURRENT_DATETIME()) AS TIMESTAMP), INTERVAL 1 DAY) AS ETL_BEGIN_EFFECTIVE_DT,
...
FROM
  ...
  `hsni-dev-edw.EDW_STG_US.STG_CLIC_SURROGKEYS` srg_key
   ...
   """
    ...
    CUSTOMER_ID
  FROM
    `hsni-dev-edw.EDW_STG_US.IM_CUSTOMER_ATTRIBUTE_REF_GCP` b"""

    ld = p | 'Read from addrorg' >> beam.io.Read(beam.io.BigQuerySource(
        query=ins_addrorg_data, use_standard_sql=True))
    lookup_data = ld | 'Gen NatKeyID ' >> beam.Map(
                lambda row: (str(row['CUSTOMER_ID'])+row['ADDR_NAME']+row['ETL_SOURCE_SYSTEM'], row))
    lookup_data2 = p1 | 'Generate UID for lookup ' >> beam.Map(
                lambda row: (
        str(row['SOURCE_CREATE_DT'])+str(row['ADDR_LINE1_TXT'])+str(row['ADDR_LINE2_TXT'])+
        str(row['CITY_NAME'])+str(row['STATE_CODE'])+str(row['COUNTRY_CODE'])+str(row['POSTAL_ZIP'])+
        str(row['POSTAL_ZIP4'])+str(row['ADDR_CLEANSING_IND'])+str(row['ADDR_FRAUD_IND'])+str(row['ADDR_QAS_VERIFIED_IND'])+
        str(row['ADDR_TYPE_CODE'])+str(row['SHIPTO_FIRST_NAME'])+str(row['SHIPTO_LAST_NAME']), row))
    ...
    p2 = (p1 | 'Filter current records' >> beam.Filter(
          ...
          | 'Lookup2' >> beam.Map(lookup2, AsDict(lookup_data2))
          | 'Filter Nones 2' >> beam.ParDo(filter_out_nones)
          )
...
FROM
  `hsni-dev-edw.EDW_STG_US.IM_CUSTOMER_ADDRESS_SCD_GCP`
WHERE
  ETL_CURRENT_IND = 1"""
...
END
  AS VOID_IND
FROM `hsni-dev-edw.EDW_STG_US.IM_CUSTOMER_ADDRESS_SCD_GCP`"""
    ...
    (pipelines_dictionary_4 | 'Updating Void Ind Field' >> LeftJoin(primary_pipeline_4, p4, join_pipeline_4, j4, common_key)
                            | 'Write to IM_CUSTOMER_ADDRESS_SCD_GCP' >> beam.io.WriteToBigQuery(
                                       output_table,
