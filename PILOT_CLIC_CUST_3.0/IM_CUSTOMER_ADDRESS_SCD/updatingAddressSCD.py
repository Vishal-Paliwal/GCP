...
FROM
  `automatic-asset-253215.STAGE.STG_CLIC_CUSTADDRORG` a"""
    ...
    addrs_SCD = p | 'Target Table' >> beam.io.Read(beam.io.BigQuerySource(
        query=address_scd,
    ...
    addrorg_old_update_tb = p | 'addrorg Table' >> beam.io.Read(beam.io.BigQuerySource(
        query=addr_old_data_update, use_standard_sql=True))

    pipelines_dictionary_2 = {primary_pipeline_1: addrs_SCD, join_pipeline_1: addrorg_old_update_tb}

    updated_old_data = (pipelines_dictionary_2 | 'Updating addrs Fields' >> LeftJoin(
        primary_pipeline_1, addrs_SCD, join_pipeline_1, addrorg_old_update_tb, common_key))
...
FROM
  ...
  FROM
    ...
    lookup_data = (addrs_SCD | 'Get Cust_Ids ' >> beam.Map(
                lambda row: (str(row['CUSTOMER_ID'])+row['ADDR_NAME']+row['ETL_SOURCE_SYSTEM'], row))
                   )

    primary_pipeline_2 = 'new_ins_data'
    new_ins_data = (p | 'Read from custorgext' >> beam.io.Read(beam.io.BigQuerySource(
        query=address_scd_query,use_standard_sql=True))
    ...
    pipelines_dictionary = {primary_pipeline_2:new_ins_data,join_pipeline_2:attribute_ref_table}

    new_ins_data_2 = (pipelines_dictionary
     | 'Left join' >> LeftJoin2(primary_pipeline_2,new_ins_data,join_pipeline_2,attribute_ref_table,common_key)
     | 'Filter Nulls' >> beam.Filter(filter_null))

    ((updated_old_data, new_ins_data_2) | 'Merge PCollections' >> beam.Flatten()
